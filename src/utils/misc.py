...
class AnnotationBlockStack:
    ...
    def find_blocks_of_clusters(self, ids: Iterable[int], oldest: bool = True, return_indices: bool = False) \
        ...
        for cluster in ids:
            ...
            selected_block_index = sorted(block_indices_this_cluster,
                                          key=lambda x: asc_desc_sign * self.blocks[x].start)[0]
    ...
    def process(self, token, cluster: Union[int, Tuple[int]], span_id: int) -> Optional[List[AnnotationBlock]]:
        """
        ...
        for new_annotation in self.subtraction(cluster, intersection):
            ...
            to_add.append(AnnotationBlock(start=span_id, words=[token], tag=new_annotation))

        # Elements in `expected - (expected ∩ cluster)` are now complete annotations, to be popped
        to_del: List[int] = self.find_blocks_of_clusters(self.subtraction(expected, intersection),
                                                         oldest=False, return_indices=True)
