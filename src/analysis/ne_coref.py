...
def _get_exact_match_ners_(
        key: List[int], ners: List[List[Union[str, int]]]
) -> List[int]:
    """If an element in spans is completely subsumed by the span in key, we return it."""
    popids: List[int] = []
    for i, ner in enumerate(ners):
...
def review(
        clustered_spans: Dict[int, list],
        clustered_spans_: Dict[int, list],
        ners,
        ners_,
        doc: Document,
        ent_src: str,
    ...
    print(
        f"Amongst {n_total} entities, {n_clustered} have been assigned to a coref cluster."
    )
    print("-------Clustered-------")
    ...
    for ent, ent_ in zip(ners, ners_):
        ...
        for cluster_id, (cluster_spans, cluster_texts) in enumerate(
                zip(doc.coref.spans, doc.coref.words)
        ):
            # Calculate fuzzy string sim
            matches[cluster_id] = sorted(
                [
                    (
                        fuzz.token_set_ratio(" ".join(ent_[1:]), " ".join(text)),
                        " ".join(text),
                        span,
                    )
...
def match_entities_to_coref_clusters(
        doc: Document, spacy_doc: Doc, ent_src: str, filter_entities: bool
    ...
    ners_chunks_lemmatized = [
        lemmatize(span, lemmas) if span != [-2, -1] else span for span in ners_chunks
    ]
    ...
    if not (
            ...
            == len(ners_filtered)
            == len(ners_filtered_)
            == len(ners_filtered_chunks)
            == len(ners_filtered_chunks_)
            == len(ners_chunks)
            == len(ners_chunks_)
            == len(ners_chunks_lemmatized)
            == len(nertags)
    ):
        ...
        print(
            f"ners chunks filtered: {len(ners_filtered_chunks)}, {len(ners_filtered_chunks_)}"
        )
        print(f"ners chunks lemmatized: {len(ners_chunks_lemmatized)}")
        print(f"ners tags: {len(nertags)}")
        raise AssertionError(
            "We are not counting the leftover entities correctly somewhere. "
        )

    clustered_span_indices = {
        _id: [getattr(doc, ent_src).spans.index(span) for span in _ents]
...
def get_ungrounded_clusters(doc: Document) -> List[int]:
    ...
    return [
        i
        for i, cluster_pos in enumerate(doc.coref.pos)
        if not ("NN" in cluster_pos or "NNP" in cluster_pos)
    ]
...
def run(split: str, entity_source: str, filter_named_entities: bool, debug: bool):
    ...
    for i, doc in enumerate(tqdm(ds)):
        ...
        summary["named_entities_per_doc"].append(
            count_doc_n_entities(doc, ent_src=ent_src)
        )
        ...
        summary["ungrounded_clusters_per_doc"][doc.docname] = get_ungrounded_clusters(
            doc
        )
        ...
        matched_entities = {
            k: [getattr(doc, ent_src).words[i] for i in v]
        ...
        summary["named_entities_unmatched_per_tag"] += [
            tupl[0] for tupl in unmatched_entities
        ]
        summary["named_entities_matched_per_tag"] += [
            ent[0] for entities in matched_entities.values() for ent in entities
        ]
        summary["named_entities_matched_per_cluster"] += [
            len(v) for k, v in matched_entity_ids.items()
        ]
        ...
        summary["clusters_matched_different_tags_per_doc"].append(
            clus_matched_diff_tags_per_doc
        )
